...
- **当前版本阶段**：`v0.6.4 — gpt-image-2 真实图像接入（Image Video 路线）`
- **已完成**：
...
| **v0.6.4** | gpt-image-2 真实图像生成接入（每张 slide 调 image2 拿底图，Pillow 仅负责文字叠加；ThreadPoolExecutor 并发；per-slide failure fallback 到 Pillow 几何图）（当前版本） |
